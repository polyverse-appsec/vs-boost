...
suite("Load Code File Command", function () {
    ...
    test("Load Code File Command Test", async function () {
        ...
        showOpenDialogMock
            ...
            .returns(Promise.resolve([vscode.Uri.file(randomFile)]));

        try {
            // Execute the "loadCodeFile" command
            await vscode.commands
                .executeCommand(NOTEBOOK_TYPE + ".loadCodeFile")
                .then(async () => {
                    // Wait for the file to be loaded
                    await new Promise((resolve) => setTimeout(resolve, 2000)); // 5 seconds to make sure file is loaded
                });
