...
export class BoostExtension {
    ...
    async loadAllSourceLevelErrorsFromNotebook(
        ...
        for (const value of this.kernels.values()) {
            ...
            if (notebook instanceof boostnb.BoostNotebook) {
                await this.buildIssueSummary(notebook as boostnb.BoostNotebook, value);
            }
            */
        }
    }

    async buildIssueSummary(notebook: boostnb.BoostNotebook, controller: FunctionKernelControllerBase) {
        ...
        boostLogging.debug(`${JSON.stringify(files)}`);

        // get the top 20% of issues (by severity)
        const topSeverityIssues = this.getTopSeverityIssues(controller.sourceLevelIssueCollection);
        if (topSeverityIssues === "[]") {
            return;
        }
        boostLogging.debug(`${controller.outputType} Top Severity Issues:\n\n${topSeverityIssues}`);

        const categorizedData = this.getTableOfIssuesBySeverityAndCategory(controller.sourceLevelIssueCollection);
        boostLogging.debug(`${controller.outputType} Categorized Issues:\n\n${categorizedData}`);
